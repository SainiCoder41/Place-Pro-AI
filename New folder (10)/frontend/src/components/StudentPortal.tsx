...
export default function StudentPortal({
  ...
  const [targetJobDesc, setTargetJobDesc] = useState(
    "Required hands-on training with React, vanilla Javascript, state hooks, CSS grid/flex, styling frameworks like Tailwind, and version control.",
  );
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    {
      ...
      role: "model",
      content:
        "Hello! I am your AI Career Assistant. I can help you with campus placement tips, mock interview roadmaps, resume feedback, and learning guides. Ask me anything!",
  ...
  const [interviewRoleTitle, setInterviewRoleTitle] = useState(
    "Software Development Intern",
  );
  const [interviewRoleDesc, setInterviewRoleDesc] = useState(
    "Good grasp of programming concepts (such as arrays, loops, OOP) and comfortable drafting modular functions.",
  );
  ...
  const [interviewQuestions, setInterviewQuestions] = useState<
    { id: number; question: string; conceptsTested: string }[]
  >([]);
  ...
  const [notifications, setNotifications] = useState([
    {
      id: "n1",
      title: "Dean approved Google recruitment schedules",
      time: "2h ago",
      unread: true,
    },
    {
      id: "n2",
      title: "Placement verification request reviewed",
      time: "1 day ago",
      unread: false,
    },
    {
      id: "n3",
      title: "System updated: ATS crawler parsing performance enhanced",
      time: "2 days ago",
      unread: false,
    },
  ]);
  ...
  useEffect(() => {
    ...
    setResumeTextInput(profile.resumeText || "");
  }, [profile]);
  ...
  const handleAnalyzeResume = async () => {
    ...
    setIsAnalyzing(true);
    try {
      const res = await fetch(`${API_BASE}/ai/gemini/analyze-resume`, {
        method: "POST",
        headers: {
      ...
      if (analysisData.atsScore) {
        onUpdateProfile({
          ...
          resumeScore: analysisData.resumeScore || analysisData.atsScore,
          readinessScore: analysisData.atsScore + 4,
          resumeText: resumeTextInput,
        });
      }
    } catch (e) {
  ...
  const handleSendChatMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/ai/gemini/chatbot`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("placely_token")}`,
  ...
  const handleGenerateQuestions = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/ai/gemini/mock-interview/generate-questions`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleEvaluateMock = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/ai/gemini/mock-interview/evaluate`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("placely_token")}`,
  ...
  const studentApps = applications.filter(
    (app) => app.studentId === profile.id,
  );
  const appliedCount = studentApps.length;
  const shortlistedCount = studentApps.filter(
    (app) =>
      app.status === "Shortlisted" ||
      app.status === "MockCompleted" ||
      app.status === "Selected",
  ).length;
  const interviewCount = mockInterviews.length;
  const offerCount =
    studentApps.filter((app) => app.status === "Selected").length ||
    (appliedCount > 0 ? 1 : 0);
      ...
      <aside className="lg:col-span-3 bg-[#E3ECE1] border-r border-[#D4DFD2] p-6 flex flex-col justify-between overflow-y-hidden">
        <div className="flex flex-col gap-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-serif text-lg font-black tracking-tight text-[#1A301E] leading-none">
                PlaceAI
              </h2>
              <span className="text-[9px] font-mono tracking-wider text-emerald-800 uppercase font-semibold mt-1 block">
                Student Space
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-5">
            <div>
              <span className="text-[10px] font-mono tracking-widest font-bold text-emerald-900/60 uppercase block mb-2.5">
                Overview
              </span>
            ...
            <div>
              ...
              <nav className="flex flex-col gap-1">
                ...
                >
                  ...
                  <span className="bg-red-500 text-white rounded-full text-[9px] px-1.5 py-0.2">
                    1
                  </span>
                </button>
                <button
        ...
        <div className="bg-white/80 border border-[#C6D6C4] p-3 rounded-2xl flex items-center gap-3 shadow-xs">
          ...
          <div className="overflow-hidden">
            <h4 className="text-xs font-bold text-[#1A301E] truncate font-sans leading-tight">
              {profile.name}
            </h4>
            <p className="text-[10px] text-[#4d614b] truncate">
              {profile.academic.degree} {profile.academic.major} · 2025
            </p>
          </div>
        </div>
      </aside>
  ...
  "
                ...
                <p className="text-xs text-neutral-500 mt-1">
                  {filteredJobs.length > 0
                    ? `${filteredJobs.length} active campus vacancies`
                    : "No direct drives found"}{" "}
                  · Last updated 2h ago
                </p>
              </div>
            ...
            <div className="bg-white rounded-3xl p-6 md:p-8 border border-neutral-300/40 shadow-sm relative overflow-hidden flex flex-col md:flex-row justify-between gap-6 items-start md:items-center">
              <div className="space-y-4 max-w-xl">
                ...
                <p className="text-xs text-neutral-500 leading-relaxed font-sans font-medium">
                  Your Python, React and Tailwind coding skills align
                  exceptionally well. Adding practical{" "}
                  <strong>DSA (Data Structures)</strong> and{" "}
                  <strong>System Design</strong> will push your candidate
                  compatibility index to 98% for immediate shortlisted priority.
                </p>

                <div className="pt-2">
                  <ShimmerButton
                    onClick={() => {
                      const tcsJob = jobs.find(
                        (j) =>
                          j.recruiterName.toLowerCase().includes("tcs") ||
                          j.recruiterName.toLowerCase().includes("tata"),
                      );
    ...
    "
              ...
              <div className="bg-[#E3ECE1]/70 border border-[#CDE1CA] rounded-2xl p-6 text-center w-full md:w-44 shrink-0 shadow-xs flex flex-col justify-center items-center">
                <span className="text-5xl font-serif font-black text-[#1A301E]">
                  94%
                </span>
                <span className="text-[10px] font-mono font-bold uppercase text-emerald-900 mt-2 block tracking-wider">
                  AI Match Score
                </span>
                <span className="text-[9px] text-[#4d614b] mt-1 block">
                  Top 8% of batch applicants
                </span>
              </div>
            </div>
    ...
    "
        ...
        w-10 h-10
        rounded-xl
        bg-[#E2F1E1] dark:bg-emerald-500/10
        border border-[#CFECCB] dark:border-emerald-500/20
...
max-h-[85%]object-contain"
                    ...
                    <Briefcase className="w-5 h-5 text-[#4A7C59] dark:text-emerald-300" />
                  )}
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
              <div className="md:col-span-8 bg-white rounded-3xl p-6 border border-neutral-300/30 shadow-xs flex flex-col gap-4">
                <div className="flex items-center justify-between border-b border-neutral-100 pb-3">
                  <h3 className="font-serif text-md font-bold text-slate-800">
                ...
                <div className="flex flex-col gap-3 max-h-[300px] overflow-y-auto">
                  {jobs.slice(0, 3).map((job) => {
                    const hasApplied = applications.some(
                      (app) =>
                        app.jobId === job.id && app.studentId === profile.id,
                    );
                    return (
                      <div
                        key={job.id}
                        className="p-3 border border-neutral-100 rounded-xl flex justify-between items-center hover:bg-neutral-50 transition"
                      >
                        <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-3">
                          <span className="text-[10px] font-mono font-semibold bg-neutral-100 px-2 py-0.5 rounded text-neutral-600">
                            {job.salary}
                          </span>
                          {hasApplied ? (
  ...
  "
                    ...
                    <h3 className="text-xs font-bold text-[#5C4D11] uppercase tracking-wider font-sans">
                      AI career advisor
                    </h3>
                  </div>

                  <p className="text-xs text-neutral-600 leading-relaxed font-sans">
                    Based on your target matches, strengthening{" "}
                    <strong>DSA (Arrays & Trees)</strong> and{" "}
                    <strong>System Design</strong> will boost recruiter search
                    visibility by <strong>35%</strong>.
                  </p>
                </div>
  ...
  "
                ...
                <p className="text-xs text-neutral-500 mt-1">
                  Please ensure your qualifications are updated accurately prior
                  to seeking placement verification reviews.
                </p>
              </div>
  ...
  "
            ...
            <h3 className="font-serif text-md font-bold text-slate-800 pt-4 border-t border-neutral-100">
              Academic Background
            </h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                {skillsList.map((skill, index) => (
                  <span
                    key={index}
                    className="bg-neutral-100 text-slate-700 px-3 py-1.5 rounded-lg text-xs font-medium border border-neutral-200 flex items-center gap-1.5 hover:bg-neutral-200 transition"
                  >
                    {skill}
                    <button
                      type="button"
            ...
            <h3 className="font-serif text-md font-bold text-slate-800 pt-4 border-t border-neutral-100">
              Academic & Self-Taught Projects
            </h3>
            <div className="flex flex-col gap-6">
              <form
                onSubmit={handleAddProject}
                className="bg-neutral-50/50 p-4 border border-neutral-200/50 rounded-xl flex flex-col gap-3"
              >
              ...
              <div className="grid grid-cols-1 gap-3 max-h-[300px] overflow-y-auto">
                {projectsList.map((p, index) => (
                  <div
                    key={index}
                    className="p-4 border border-neutral-200 rounded-xl relative flex justify-between items-start bg-neutral-50/20 hover:bg-neutral-50/60 transition"
                  >
                    <div>
                      <h4 className="text-xs font-bold text-slate-800 flex items-center gap-2">
                        {p.name}
                        {p.tech && (
                          <span className="font-mono text-[10px] bg-[#E1EFE0] text-slate-700 border border-neutral-200 py-0.5 px-1.5 rounded">
                            {p.tech}
                          </span>
                        )}
                      </h4>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-semibold text-neutral-600 block">
                Paste or edit your full resume content (for ATS scoring and mock
                interviews)
              </label>
              <textarea
                ...
                rows={6}
                className="w-full text-xs bg-neutral-50 border border-neutral-200 rounded-xl p-3 font-mono focus:outline-none focus:ring-1 focus:ring-[#A0C595] focus:bg-white transition"
              />
            </div>
          </div>
        )}
    ...
    "
              ...
              <p className="text-xs text-neutral-500 mt-1">
                Audit your resume matching dynamic industry roles. Get immediate
                score diagnostics, spot missing critical skills, and review
  ...
  <div>
    <label className="text-xs font-bold text-neutral-600 block mb-1">
      Required Skills Guidelines / Job Description
    </label>
    ...
    />
            ...
            <div>
              <label className="text-xs font-bold text-neutral-700 block mb-1">
                Copy Your Full Resume Content (Paste Text)
              </label>
              <textarea
                value={resumeTextInput}
                onChange={(e) => setResumeTextInput(e.target.value)}
...
>
  ...
  <span className="flex items-center gap-2">
    ...
    Run AI ATS Audit & Screen Resume
  </span>

  {/* Right Corner Loader */}
  {isAnalyzing && (
    <span className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2 text-xs opacity-90">
      <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
            ...
            {analysisResult && (
              <div className="mt-4 pt-6 border-t border-neutral-100 dark:border-neutral-700 flex flex-col gap-6 text-neutral-800 dark:text-white">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-6 flex flex-col items-center justify-center text-center">
                    <span className="text-xs text-emerald-800 font-bold uppercase tracking-wider mb-2">
                      ATS Score
                    </span>
                    <div className="text-4xl font-mono font-extrabold text-[#1A301E]">
                      {analysisResult.atsScore}%
                    </div>
                    <span className="text-[10px] text-emerald-700 mt-2">
                      Compatible with modern scanners
                    </span>
                  </div>
                  ...
                  <div className="bg-neutral-100 border border-neutral-200 rounded-xl p-6 flex flex-col items-center justify-center text-center">
                    <span className="text-xs text-neutral-600 font-bold uppercase tracking-wider mb-2">
                      Resume Score
                    </span>
                    <div className="text-4xl font-mono font-extrabold text-[#1D352D]">
                      {analysisResult.resumeScore}%
                    </div>
                    <span className="text-[10px] text-neutral-500 mt-2">
                      Evaluation of content phrasing
                    </span>
                  </div>

                 <div className="bg-amber-50 dark:bg-amber-900/20 border border-[#A0C595] dark:border-amber-700 rounded-xl p-6 flex flex-col items-center justify-center text-center">
  <span className="text-xs text-amber-900 dark:text-amber-200 font-bold uppercase tracking-wider mb-2">
    Readiness Verdict
  </span>

  <div className="text-xl font-bold text-[#5C4D11] dark:text-white">
    {analysisResult.readinessRating}
  </div>

  <span className="text-[10px] text-amber-700 dark:text-amber-300 mt-2">
    Matching standard parameters
  </span>
</div>
                </div>
                ...
                <div className="p-4 bg-[#E3ECE1] border border-[#A0C595] rounded-xl">
                  <h4 className="text-xs font-bold text-slate-800 mb-1">
                    AI Industry Placement Brief
                  </h4>
                  <p className="text-xs text-slate-700 leading-relaxed font-sans">
                    {analysisResult.industryReadinessSummary}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="p-4 bg-white border border-neutral-200 rounded-xl shadow-xs">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {analysisResult.skillGapAnalysis?.detectedSkills?.map(
                        (s: string, i: number) => (
                          <span
                            key={i}
                            className="bg-emerald-50 text-[10px] py-1 px-2 rounded-md font-medium text-emerald-700"
                          >
                            {s}
                          </span>
                        ),
                      ) || (
                        <span className="text-neutral-400 text-xs text-center w-full">
                          ---
                        </span>
                      )}
                    </div>
                  ...
                  <div className="p-4 bg-white border border-neutral-200 rounded-xl shadow-xs">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {analysisResult.skillGapAnalysis?.missingCriticalSkills?.map(
                        (s: string, i: number) => (
                          <span
                            key={i}
                            className="bg-red-50 text-[10px] py-1 px-2 rounded-md font-medium text-red-700"
                          >
                            {s}
                          </span>
                        ),
                  ...
                  <div className="p-4 bg-white border border-neutral-200 rounded-xl shadow-xs">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {analysisResult.skillGapAnalysis?.goodToHaveMissing?.map(
                        (s: string, i: number) => (
                          <span
                            key={i}
                            className="bg-blue-50 text-[10px] py-1 px-2 rounded-md font-medium text-blue-700"
                          >
                            {s}
                          </span>
                        ),
                ...
                <div className="bg-neutral-50 border border-neutral-200 rounded-xl p-5 flex flex-col gap-4">
                  <h4 className="text-xs font-bold text-neutral-800">
                    Direct Improvement Checklist
                  </h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <h5 className="text-xs font-semibold text-neutral-700 mb-2 font-mono">
                        Formatting & Structure
                      </h5>
                      <ul className="list-disc pl-4 flex flex-col gap-1.5 text-[11px] text-neutral-600 leading-relaxed font-sans">
                        {analysisResult.improvementSuggestions?.formatting?.map(
                    ...
                    <div>
                      ...
                      <ul className="list-disc pl-4 flex flex-col gap-1.5 text-[11px] text-neutral-600 leading-relaxed font-sans">
                        {analysisResult.improvementSuggestions?.keywords?.map(
                          (k: string, idx: number) => <li key={idx}>{k}</li>,
  ...
  "
    ...
    "
              ...
              <p className="text-xs text-neutral-500 mt-1">
                Simulate real-time corporate technical, behavioral, or HR
                interview cycles. Solve customized questions, type your answers,
                and obtain comprehensive grading metrics generated server-side
                by Groq.
              </p>
            </div>
  ...
  "
                    ...
                    <input
                      ...
                      placeholder="e.g. Frontend Developer · Backend Engineer · Data Analyst"
                      className="
            w-full
  ...
  "
        ...
        "
                  >
                    Skills, Technologies & Job Expectations
                  </label>
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Mention required skills, technologies, experience level, responsibilities, or interview focus areas..."
                    className="
          w-full
              ...
              <div className="bg-neutral-50/50 rounded-2xl p-4 md:p-6 border border-neutral-200/50 flex flex-col gap-6">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest">
                    Question {activeQuestionIdx + 1} of{" "}
                ...
                <div className="bg-white border border-indigo-200/50 rounded-2xl p-6 shadow-sm">
                  <span className="text-[10px] font-bold text-indigo-700 bg-indigo-50 border border-indigo-100 px-2 py-1 rounded inline-block">
                    TESTING FOR:{" "}
                    {interviewQuestions[activeQuestionIdx].conceptsTested}
                  </span>
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-xs font-bold text-neutral-600">
                    Draft Your Professional Verbal/Technical Response
                  </label>
                  <textarea
                    value={
                      answersMap[interviewQuestions[activeQuestionIdx].id] || ""
                    }
                    onChange={(e) =>
                      handleAnswerSubmit(
                        interviewQuestions[activeQuestionIdx].id,
                        e.target.value,
                      )
                    }
            ...
            {interviewReport && (
              <div className="flex flex-col gap-6">
                <div className="bg-neutral-50 p-6 border border-neutral-200 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-6">
                  <div>
                    <h3 className="font-serif text-lg font-bold text-[#1A301E]">
                      AI Panel Grade Report
                    </h3>
                    <p className="text-xs text-neutral-500 mt-0.5">
                      Your answer transcript was parsed and graded directly
                  ...
                  <div className="flex items-center gap-4">
                    <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-3 px-5 text-center">
                      <span className="text-[10px] text-indigo-700 block uppercase tracking-wider font-bold">
                        Grade
                      </span>
                      <span className="text-4xl font-serif font-black text-indigo-900">
                        {interviewReport.grade}
                      </span>
                    </div>
                    <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3 px-5 text-center">
                      <span className="text-[10px] text-emerald-700 block uppercase tracking-wider font-bold">
                        Total Index
                      </span>
                      <span className="text-4xl font-mono font-black text-emerald-950">
                        {interviewReport.totalScore}%
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 bg-indigo-50/50 border border-indigo-100 rounded-xl">
                  <h4 className="text-xs font-bold text-indigo-950 mb-1">
                    AI Panel Verdict Card
                  </h4>
                  <p className="text-xs text-indigo-900 font-medium leading-relaxed font-sans">
                    {interviewReport.readinessVerdict}
                  </p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-emerald-50/10 p-5 rounded-xl border border-emerald-100">
                    <h4 className="text-xs font-bold text-emerald-800 mb-3 block">
                      Detected Strengths
                    </h4>
                    <ul className="list-disc pl-4 text-xs text-slate-700 flex flex-col gap-2 font-sans font-medium">
                      {interviewReport.strengths?.map(
                        (str: string, sIdx: number) => (
                          <li key={sIdx}>{str}</li>
                        ),
                      ) || <li>Strong effort overall</li>}
                    </ul>
                  </div>

                  <div className="bg-red-50/10 p-5 rounded-xl border border-red-100">
                    <h4 className="text-xs font-bold text-red-800 mb-3 block">
                      Recommended Improvements
                    </h4>
                    <ul className="list-disc pl-4 text-xs text-slate-700 flex flex-col gap-2 font-sans font-medium">
                      {interviewReport.weaknesses?.map(
                        (wk: string, wIdx: number) => <li key={wIdx}>{wk}</li>,
                      ) || <li>No glaring weaknesses found</li>}
                    </ul>
                  </div>
                </div>

                <h4 className="text-xs font-bold text-slate-800 pt-4 border-t border-neutral-100 font-mono text-center">
                  Question-by-Question Diagnostics Analysis
                </h4>
                <div className="space-y-4 max-h-[400px] overflow-y-auto">
                  {interviewReport.detailedQnaFeedback?.map(
                    (qItem: any, idx: number) => (
                      <div
                        key={idx}
                        className="bg-white rounded-2xl p-5 border border-neutral-200/50 flex flex-col gap-3 shadow-xs"
                      >
                        <div className="flex justify-between items-center border-b border-neutral-100 pb-2">
                          <span className="text-[10px] font-bold text-neutral-400 block tracking-wider">
                            INQUIRY {qItem.questionId}
                          </span>
                          <span className="font-mono text-xs font-bold bg-white border border-neutral-200 py-1 px-2.5 rounded-lg text-emerald-700">
                            {qItem.score}/100
                          </span>
                        </div>
                        ...
                        <p className="text-xs italic bg-neutral-50 p-3 rounded-lg text-neutral-600 font-sans">
                          Your Answer: {qItem.candidateAnswer || "(Left Blank)"}
                        </p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs mt-1">
                          ...
                          <div className="bg-red-50/20 p-3 rounded-xl border border-red-100/20">
                            <span className="text-[10px] font-bold text-red-800 uppercase tracking-widest block mb-1">
                              Cons
                            </span>
                            <span className="text-[11px] text-red-950 block">
                              {qItem.cons}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="bg-indigo-50/30 border border-indigo-100/50 p-4 rounded-xl mt-1">
                          <span className="text-[10px] font-bold text-indigo-800 uppercase tracking-widest block mb-1">
                            Elite Model Answer Blueprint
                          </span>
                          <p className="text-[11px] text-slate-700 leading-relaxed font-serif">
                            {qItem.suggestedExcellentModelAnswer}
                          </p>
                        </div>
                      </div>
                    ),
                ...
                <div className="bg-indigo-900 text-white rounded-2xl p-6 flex flex-col md:flex-row items-center gap-6 mt-4">
                  ...
                  <div>
                    <h4 className="font-bold text-indigo-200 text-xs uppercase tracking-wider">
                      AI Panel Placement Blueprint Recommendations
                    </h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                      {interviewReport.actionableTips?.map(
                        (tip: string, tIdx: number) => (
                          <div
                            key={tIdx}
                            className="flex gap-2 items-start text-xs text-neutral-200"
                          >
          ...
          <div className="bg-white rounded-2xl border border-neutral-200/50 shadow-sm flex flex-col h-[550px] overflow-hidden">
            <div className="p-4 md:p-5 border-b border-neutral-100 bg-neutral-50/50 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h2 className="text-sm font-bold text-slate-800 font-serif">
                    AI Placement Advisor Guidance Desk
                  </h2>
                  <p className="text-[10px] text-neutral-500">
                    Provides continuous learning maps and campus placement
                    tactics
                  </p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 bg-neutral-50/25">
              {chatMessages.map((msg) => (
                ...
                >
                  <p className="text-[9px] text-neutral-400 mb-1 px-1">
                    {msg.timestamp}
                  </p>
                  <div
            ...
            <form
              ...
              className="p-4 border-t border-neutral-100 flex gap-2 bg-white"
            >
              <input
                type="text"
                value={chatInput}
  ...
  "
              >
                Approved Campus Hiring Opportunities
              </h2>
              <p className="text-xs text-neutral-500 mt-1">
                Explore current verification-passed vacancies. Verified students
                can instantly click 'Apply Now' to lock applications onto
                recruiter files with complete profiles.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              {filteredJobs.map((job) => {
                const hasApplied = applications.some(
                  (app) => app.jobId === job.id && app.studentId === profile.id,
                );
                const isVerified_user =
                  profile.verificationStatus === "Verified";
                const btnText = hasApplied
                ...
                return (
                  <div
                    key={job.id}
                    className="bg-white rounded-2xl p-6 border border-neutral-200/50 hover:border-neutral-300 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-5 transition-all"
                  >
                    <div>
  ...
  "
                      ...
                      <p className="text-xs text-neutral-500 mt-1 max-w-2xl line-clamp-2 leading-relaxed font-sans">
                        {job.description}
                      </p>
                      ...
                      <div className="flex flex-wrap gap-2 mt-3.5">
                        {job.skills.map((s, sIdx) => (
                          <span
                            key={sIdx}
                            className="bg-neutral-100 text-[10px] text-neutral-700 font-medium py-1 px-2.5 rounded-md"
                          >
                            {s}
                          </span>
                        ))}
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4 pt-4 border-t border-neutral-100 text-xs text-neutral-500">
                        <div>
                          <span className="text-[10px] uppercase block tracking-wider text-neutral-400 font-mono">
                            Salary Package
                          </span>
                        ...
                        <div className="col-span-2">
                          <span className="text-[10px] uppercase block tracking-wider text-neutral-400 font-mono font-medium">
              ...
              {filteredJobs.length === 0 && (
                <div className="p-8 text-center bg-white border border-neutral-200 rounded-2xl text-neutral-400 text-xs">
                  No vacancies matching search details. Clear search input or
                  wait for campus officer approvals.
                </div>
              )}
            </div>
          </div>
  ...
  "
              ...
              <p className="text-xs text-neutral-500 mt-1">
                Check progress logs of submitted campus drives. Statuses lock
                automatically when reviews register during recruiter updates.
              </p>
            </div>
            ...
            <div className="space-y-4">
              {studentApps.map((app) => {
                ...
                return (
                  <div
                    key={app.id}
                    className="p-4 border border-neutral-100 rounded-xl flex justify-between items-center bg-neutral-50/20"
                  >
                    <div className="space-y-1">
                      <h4 className="text-xs font-bold text-slate-800">
                        {job?.title || "Hiring Vacancy"}
                      </h4>
                      <span className="text-[10px] text-neutral-400 font-mono block">
                        {job?.recruiterName || "Company Name"} · Applied on{" "}
                        {app.appliedDate}
                      </span>
                    </div>
              ...
              {studentApps.length === 0 && (
                <div className="p-8 text-center bg-white border border-neutral-200 rounded-2xl text-neutral-400 text-xs">
                  Your pipeline has no submissions. Jump to "Drives" tab and
                  select suitable opportunities!
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 md:p-8 border border-neutral-200/50 shadow-sm flex flex-col gap-6 max-h-[750px] overflow-y-auto">
            <div>
              <h2 className="font-serif text-lg font-bold text-slate-800">
                Your Placement Metric Analytics
              </h2>
              <p className="text-xs text-neutral-500 mt-1">
                Real-time candidate indexing metrics compiled server-side.
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="p-5 border border-neutral-200 rounded-xl space-y-4">
                <h4 className="text-xs font-bold text-slate-800 font-mono block">
                  QUANTITATIVE RATING INDEXES
                </h4>
                <div className="space-y-3">
                  <div>
                    <div className="flex justify-between items-center text-xs mb-1">
                      <span className="text-neutral-500">
                        ATS Resume Filter Compliance
                      </span>
                    ...
                    <div className="w-full bg-neutral-100 h-2 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${profile.atsScore || 70}%` }}
                      ></div>
                    </div>
                  </div>
                  <div>
                    <div className="flex justify-between items-center text-xs mb-1">
                      <span className="text-neutral-500">
                        Academic Standings (CGPA Map)
                      </span>
                      <span className="font-bold text-slate-700">
                        {(parseFloat(profile.academic.cgpa) || 8.5) * 10}%
                      </span>
                    </div>
                    <div className="w-full bg-neutral-100 h-2 rounded-full overflow-hidden">
                      <div
                        className="bg-[#A0C595] h-full"
                        style={{
                          width: `${(parseFloat(profile.academic.cgpa) || 8.5) * 10}%`,
                        }}
                      ></div>
                    </div>
                  </div>
                  <div>
                    <div className="flex justify-between items-center text-xs mb-1">
                      <span className="text-neutral-500">
                        Calculated Interview Readiness Index
                      </span>
                      <span className="font-bold text-slate-700">
                        {profile.atsScore ? profile.atsScore + 4 : 74}%
                      </span>
                    </div>
                    <div className="w-full bg-neutral-100 h-2 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${profile.atsScore ? profile.atsScore + 4 : 74}%`,
                        }}
                      ></div>
                    </div>
                  </div>
              ...
              <div className="p-5 border border-neutral-200 rounded-xl flex flex-col justify-between">
                <div>
                  <h4 className="text-xs font-bold text-slate-800 font-mono block mb-2">
                    CAMPUS BENCHMARK PLACEMENT RATINGS
                  </h4>
                  <p className="text-xs text-neutral-500 leading-relaxed font-sans mt-2">
                    Your skills in{" "}
                    <strong>
                      {profile.skills.slice(0, 3).join(", ") || "software"}
                    </strong>{" "}
                    put you in the elite <strong>Top 15%</strong> of applicants
                    on campus. Verify your CGPA with College authorities to
                    unlock premium drives instantly.
                  </p>
                </div>
        ...
        {activeTab === "notifications" && (
          <div className="bg-white rounded-2xl p-6 md:p-8 border border-neutral-200/50 shadow-sm flex flex-col gap-6 max-h-[750px] overflow-y-auto">
            <div>
              <h2 className="font-serif text-lg font-bold text-slate-800">
                Placement Office Alerts & Announcements
              </h2>
              <p className="text-xs text-neutral-500 mt-1">
                Get immediate reminders, schedules and modifications direct from
            ...
            <div className="space-y-3">
              {notifications.map((item) => (
                <div
                  key={item.id}
                  className={`p-4 border rounded-xl flex justify-between items-center transition ${item.unread ? "bg-amber-50/20 border-amber-200" : "bg-neutral-50/30 border-neutral-200/50"}`}
                >
                  <div className="flex items-center gap-3">
                    <span className="text-md">{item.unread ? "🔔" : "📎"}</span>
                    <div>
                      <h4
                        className={`text-xs ${item.unread ? "font-bold text-slate-800" : "text-slate-600"}`}
                      >
                        {item.title}
                      </h4>
        ...
        {activeTab === "settings" && (
          <div className="bg-white rounded-2xl p-6 md:p-8 border border-neutral-200/50 shadow-sm flex flex-col gap-6 max-h-[750px] overflow-y-auto">
            <div>
              <h2 className="font-serif text-lg font-bold text-slate-800">
                Space Preference Settings
              </h2>
              <p className="text-xs text-neutral-500 mt-1">
                Adjust your diagnostic platform defaults and clear cached
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center p-3 border-b border-neutral-100">
                <div>
                  <h4 className="text-xs font-bold text-slate-800">
                    Enable Automated Deep-Scan ATS Resume Crawls
                  </h4>
                  <p className="text-[10px] text-neutral-400">
                    Pre-runs keyword audits prior to clicking 'Apply now'
                  </p>
                </div>
                <input
                  type="checkbox"
                  defaultChecked
                  className="rounded text-emerald-700 focus:ring-emerald-400"
                />
              </div>
              ...
              <div className="flex justify-between items-center p-3 border-b border-neutral-100">
                <div>
                  <h4 className="text-xs font-bold text-slate-800">
                    Show Placement Mock Match Pill Alerts
                  </h4>
                  <p className="text-[10px] text-neutral-400">
                    Dispatches overlay popups identifying active hiring matches
                  </p>
                </div>
                <input
                  type="checkbox"
              ...
              <div className="p-3 bg-red-50/20 border border-red-200 rounded-xl">
                <h4 className="text-xs font-bold text-red-800">
                  Clear Simulated Session Memory
                </h4>
                <p className="text-[10px] text-red-700/80 mt-1">
                  Will clear candidate records, simulated application statuses,
                  and interview scores back to original seed defaults.
                </p>
                <div className="mt-3">
                  <button
                    onClick={() => {
                      if (
                        confirm("Are you sure you want to restore defaults?")
                      ) {
